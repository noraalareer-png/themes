...
const coverage = read(`${base}/coverage.json`);   // section coverage (from the preset), if produced
...
// Section-coverage bits (only rendered if coverage.json exists for this run).
const covHasGap = coverage && Array.isArray(coverage.unplaced) && coverage.unplaced.length > 0;
const coverageCard = coverage ? `
  <div class="card"><div class="big" style="color:${covHasGap ? '#b7791f' : '#2f855a'}">${coverage.placed_count}/${coverage.shipped_count}</div><div class="muted">تغطية الأقسام</div></div>` : '';
const coverageNote = coverage ? `
<p class="muted">تغطية الأقسام (من الـpreset): <b>${coverage.placed_count}/${coverage.shipped_count} (${coverage.coverage_pct}%)</b>
${covHasGap
    ? ' · أقسام يشحنها الثيم وغير موضوعة في الـpreset (مُختبَرة معزولة فقط عبر /validate): <b>' + esc(coverage.unplaced.join(', ')) + '</b>'
    : ' · ✓ كل الأقسام المشحونة موضوعة في الـpreset'}</p>` : '';
...
<div class="sum">
  ...
  <div class="card"><div class="big" style="color:#a0aec0">${skipped}</div><div class="muted">متخطّاة</div></div>${coverageCard}
</div>
<p class="muted">الفحص البنيوي (/validate): <b>${structuralPass===null?'لم يُشغّل':structuralPass?'مكتمل':'فيه ملاحظات'}</b>
${validate?.failing_templates?.length ? ' · تمبلتس بها ملاحظات: ' + esc(validate.failing_templates.join(', ')) : ''}</p>${coverageNote}
...
console.log(`Report written: ${outPath}  (automation ${automationOk ? 'OK' : 'incomplete'}; ${passed} passed, ${failed} to review, ${skipped} skipped${coverage ? `; sections ${coverage.placed_count}/${coverage.shipped_count}` : ''})`);
